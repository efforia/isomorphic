...
socket.on('new message', async message => {
  // When there is an new message notification
 ...
 */
Message.on('afterInsert', async created => {
  // When a new message is created
  let message = await Message.findOne({ _id: created._id }).populate('from to') // Retrieves message info
  message.to.forEach(user => {
    socket.emit('new message', Object.assign({ room: message.to }, message))
  }) // Informs the related users
  notification.send({
    // Sends an notification with:
