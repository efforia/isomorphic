...
*/
    ...
    },
    {
      ...
      info: {
        facebook:
          'For applications registered after April 2014 only friends who have used the same app appear in resultset. See https://developers.facebook.com/docs/apps/changelog'
      },
      api: 'api',
      method: 'get',
      path: 'me/friends',
      ...
      hello(test.network)
        ...
        .then(
          function(r) {
            ...
            callback(
              "Failed to setup: Create file 'TestFile.png' (there's a test above which does this)"
            )
          },
          function() {
  ...
  function get_test_folder(test, callback) {
    if (
      !('id' in test.data && test.data.id.match(/\_ID\]$/)) &&
      !('parent' in test.data && test.data.parent.match(/\_ID\]$/))
    ) {
      callback()
      return
    }
    ...
    hello(test.network)
      ...
      .then(
        function(r) {
          ...
          callback(
            "Failed to setup: First create a folder called 'TestFolder' (there's a test above which does this)"
          )
        },
        function() {
          callback(
            "Failed to setup, could not access me/folders, ensure your signed in with the 'files' scope"
          )
        }
      )
  }
  ...
  function Test(test, network, parent) {
    ...
    if (!network) {
      ...
      this.enabled =
        { login: 1, logout: 1, getAuthResponse: 1 }[this.method] ||
        (action in hello.services[network] && test.path in hello.services[network][action])
  ...
  function Provider(network) {
    ...
    this.login = function() {
      hello.login(network, { scope: model.checkedScopes() }).then(
        function() {
  ...
  self.model = new function() {
    ...
    this.hasConnected = ko.computed(function() {
      return (
        ko.utils.arrayFilter(this.networks(), function(item) {
          return item.online()
        }).length > 0
      )
    }, this)
  ...
  ko.bindingHandlers.beautify = {
    update: function(element, valueAccessor, allBindingsAccessor) {
      ...
      value = (JSON.stringify(value, fixRegExp, 1) || '')
        .replace(/https?:\/\/[^\'\"\s]+/gi, function(r) {
          return r.link(r).replace('<a ', '<a target="_blank" ')
        })
        .replace(/\"(\/.*?\/)\"/gi, function(m, a) {
